...
import com.jouryu.socket.hbase.HbaseService;
...
public class ReceiveSensorDataHandler extends SimpleChannelInboundHandler<byte[]> {
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        CacheData.tcpChannelMap.put(getChannelId(channel), channel);
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
        ...
        // 清除monitorMap对应的连接映射
        CacheData.monitorMap.remove(monitorCode);
        // 清除当前连接的channel对象
        CacheData.tcpChannelMap.remove(getChannelId(channel));
        logger.info("传感器(" + getRemoteAddress(ctx) + ")[" + CacheData.getMonitorNameByCode(monitorCode) + "]已退出处理队列");
        logger.info("当前传感器列表: [" + String.join(",", CacheData.getCurrentMonitorList()) + "]");
    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        logger.info("传感器(" + getRemoteAddress(ctx) + ")[" + CacheData.getMonitorNameByChannelId(channelId) + "]断开连接");
    }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        String channelId = getChannelId(ctx.channel());
        logger.info("传感器(" + getRemoteAddress(ctx) + ")[" + CacheData.getMonitorNameByChannelId(channelId) + "]发生异常");
    ...
    private void registerHandler(Channel channel, String message) {
        ...
        if (!monitorCode.equals("")
                && CacheData.containsMonitorCode(monitorCode)
                && !CacheData.monitorMap.containsKey(monitorCode))
        {
            // 往客户端发送一条注册成功的消息
            String command = CommandTypeEnum.REGISTER_SUCCESS.getCommand();
            channel.writeAndFlush(Utils.hexStringToBytes(command));
            // 关联monitor code和channel id
            CacheData.monitorMap.put(monitorCode, channelId);
            logger.info("当前传感器列表: [" + String.join(",", CacheData.getCurrentMonitorList()) + "]");
        }
    }
